...
class WholeSlideImage(object):
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            print("********attr_dict: *********")
            print(attr_dict)
            if len(asset_dict) >= 0: # lizx: changed to >=
                if init:
    ...
    def visHeatmap(self, 
        ...
        if binarize:
            ...
            threshold=thresh
            iprint(f"binarization is off, threshold = {thresh} (only scores greater than this will be drawn)",ident=ident_level+1)
        ...
        iprint('w: {}, h: {}'.format(w, h),ident=ident_level+2)
        ...
        if value_type=="one_part":
            if normalization_method=="convert_to_percentiles":
                ...
                if np.any(scores<0) or np.any(scores>1):
                    iprint(
                        f"scores are not within range [0,1] (mn={mn}, mx={mx}) and normalization_method is None, this may cause trouble for heatmap",
                        ident=ident_level+2
                    )
                heatmap_vis_data["normalization_max"]=1.0
                heatmap_vis_data["normalization_min"]=0.0
            ...
            if normalization_method=="convert_to_percentiles":
                ...
                iprint("using percentile normalization, the percentiles are normalized to [-1,1]", ident=ident_level+2)

            elif normalization_method=="min_max":
                
                scores,heatmap_vis_data_=self.two_parts_linear_normalization(scores,normalization_method="min_max",ident_level=ident_level+2)
                ...
                scores,heatmap_vis_data_=self.two_parts_linear_normalization(scores,normalization_method="quantile",ident_level=ident_level+2)
                heatmap_vis_data.update(heatmap_vis_data_)
                ...
                if np.any(scores<-1) or np.any(scores>1):
                    iprint(
                        f"scores are not within range [-1,1] (mn={mn}, mx={mx}) and normalization_method is None, this may cause trouble for heatmap",
                        ident=ident_level+2
                    )
                heatmap_vis_data["normalization_min_neg"]=-1
                heatmap_vis_data["normalization_max_neg"]=0
        ...
        if binarize:
            print('binarized tiles based on cutoff of {}'.format(threshold),ident=ident_level+1)
            print('identified {}/{} patches as positive'.format(count, len(coords)),ident=ident_level+1)
        ...
        iprint('total of {} patches'.format(len(coords)),ident=ident_level+2)
        ...
        for idx in range(len(coords)):
            if (idx + 1) % twenty_percent_chunk == 0:
                iprint('progress: {}/{}'.format(idx, len(coords)),ident=ident_level+2)
    ...
    def block_blending(self, img, vis_level, top_left, bot_right, alpha=0.5, blank_canvas=False, block_size=1024,ident_level=0):
        ...
        iprint('using block size: {} x {}'.format(block_size_x, block_size_y),ident=ident_level+1)
